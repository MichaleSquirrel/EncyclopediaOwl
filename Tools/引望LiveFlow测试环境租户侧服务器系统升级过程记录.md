...
lsblk
```
> `df -h` 是一个在Linux操作系统中常用的命令，用于查看磁盘的使用情况。这个命令的目的是显示系统上可用的磁盘空间。具体来说，`df` 是 "Disk Free" 的缩写，代表这个命令的功能是显示磁盘的剩余空间。`-h` 参数是一个选项，表示 "human-readable"，意味着该命令将自动选择合适的容量单位（如KB、MB、GB）来显示数据，使结果更易于阅读和理解。
>
> 下面是这个命令的详细解释：
>
> `df`: 这是命令本身，用于显示磁盘空间使用情况。
> `-h`: 这个参数表示 "human-readable"，会让输出结果更易于阅读。例如，数据可能会以KB、MB或GB为单位显示，具体取决于磁盘的大小。
>
> 这个命令的输出通常包括以下几个部分：
>
> `文件系统`: 表示磁盘或分区的名称。
> `1K-块`: 表示每个块的大小，通常为1KB。
> `已用`: 表示已经使用的磁盘空间大小。
> `可用`: 表示当前可以无损地分配给文件或目录的空间大小。
> `已用%`: 表示已用空间占总空间的百分比。
> `挂载点`: 表示这个文件系统挂载在哪个目录下。

> df -T：显示文件系统类型，选项会在输出中增加一列 "Type"，显示文件系统类型（如 ext4、xfs 等）

> lsblk命令的英文是“list block”，即用于列出所有可用块设备的信息，而且还能显示他们之间的依赖关系df
> lsblk -f：查看所有块设备及其文件系统信息，lsblk 是 list block devices 的缩写，加上 -f 参数可以显示文件系统相关信息。



可以使用 df -T 或 lsblk -f 查看文件系统类型

![2026-01-16_153949.png](pic20260104/2026-01-16_153949.png)

如果分区没有格式化，可以使用mkfs.xfs 格式化磁盘分区

> mkfs.xfs 的基本语法
> mkfs.xfs [选项] 设备路径
> -f：强制格式化（即使设备已存在文件系统或已挂载，谨慎使用！）
> -L 标签名：给文件系统设置一个易记的标签，后续挂载时可通过标签识别
> -q：安静模式，减少输出信息
...
> https://blog.csdn.net/shizheng_Li/article/details/145801445
> /etc/fstab（File System Table）是一个至关重要的配置文件，它用于定义系统开机时自动挂载的文件系统。
